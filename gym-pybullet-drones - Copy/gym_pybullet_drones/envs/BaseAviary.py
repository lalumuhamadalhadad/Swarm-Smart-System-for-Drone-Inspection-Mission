...
class BaseAviary(gym.Env):
    ...
    """def _addObstacles(self):
        ...
        p.loadURDF(r"D:\Documents\ITB\S2\Semester 2\Thesis 2\robot_scenarios_sim\models_pkg\models\kitchen\kitchen\kitchen.urdf",
                   physicsClientId=self.CLIENT
                   )
        p.loadURDF(r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Room.urdf",
                   physicsClientId=self.CLIENT
                   )
    ...
    def _addObstacles(self):
        # Load your maze/room URDF
        maze_urdf_path = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\RoomNoInterior Rectangle.urdf"
        
        try:
            self.ROOM_ID = p.loadURDF(maze_urdf_path, physicsClientId=self.CLIENT)
            print(f"[INFO] Successfully loaded maze URDF with ID: {self.ROOM_ID}")
            
            # Apply both floor and wall textures
            self._setupMazeTextures()
            
            # Print link information for debugging (only in GUI mode)
            if self.GUI:
                self._printMazeLinkInfo()
                
        except Exception as e:
            print(f"[ERROR] Failed to load maze URDF from {maze_urdf_path}: {e}")
            print("[INFO] Continuing without maze environment")

    def _setupMazeTextures(self):
        if not hasattr(self, 'ROOM_ID'):
            print("[WARNING] No room loaded, cannot apply textures")
            return
        
        print("[INFO] Setting up maze textures...")
        
        # Floor texture setup
        floor_texture_path = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\concrete-tile-background.jpg"
        floor_success = self._applyFloorTexture(floor_texture_path)
        
        # Wall texture setup
        wall_texture_path = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Walltexture.jpeg"
        wall_success = self._applyWallTextures(wall_texture_path)
        
        # Summary
        if floor_success and wall_success:
            print("[INFO] ✓ All textures applied successfully!")
        elif floor_success:
            print("[INFO] ✓ Floor texture applied, ⚠ wall texture failed")
        elif wall_success:
            print("[INFO] ⚠ Floor texture failed, ✓ wall texture applied")
        else:
            print("[WARNING] ✗ Both floor and wall textures failed to apply")

    def _applyFloorTexture(self, texture_path):
        try:
            if not os.path.exists(texture_path):
                print(f"[WARNING] Floor texture file not found: {texture_path}")
                return False
                
            # Load the texture
            texture_id = p.loadTexture(texture_path, physicsClientId=self.CLIENT)
            
            # Apply to base link (floor) - linkIndex=-1 for base link
            p.changeVisualShape(self.ROOM_ID, 
                            linkIndex=-1,  # base_link is the floor
                            textureUniqueId=texture_id,
                            rgbaColor=[1, 1, 1, 1],  # White tint for clear texture
                            physicsClientId=self.CLIENT)
            
            print(f"[INFO] ✓ Floor texture applied: {os.path.basename(texture_path)}")
            return True
            
        except Exception as e:
            print(f"[ERROR] Failed to apply floor texture: {e}")
            return False

    def _applyWallTextures(self, wall_texture_path):
        if not os.path.exists(wall_texture_path):
            print(f"[WARNING] Wall texture file not found: {wall_texture_path}")
            return False
        
        try:
            # Load wall texture (supports both PNG and JPEG)
            wall_texture_id = p.loadTexture(wall_texture_path, physicsClientId=self.CLIENT)
            print(f"[INFO] Wall texture loaded: {os.path.basename(wall_texture_path)}")
            
            # Method 1: Try expected link indices
            success_count = self._applyWallTexturesByIndex(wall_texture_id)
            
            # Method 2: If Method 1 fails, try scanning all links
            if success_count == 0:
                print("[DEBUG] Primary method failed, trying alternative approach...")
                success_count = self._applyWallTexturesByScanning(wall_texture_id)
            
            if success_count > 0:
                print(f"[INFO] ✓ Wall texture applied to {success_count} walls")
                return True
            else:
                print("[WARNING] ✗ Failed to apply wall texture to any walls")
                return False
            
        except Exception as e:
            print(f"[ERROR] Failed to load wall texture: {e}")
            return False

    def _applyWallTexturesByIndex(self, wall_texture_id):
        # Expected wall links based on URDF structure
        wall_links = {
            0: "north_wall",
            1: "south_wall", 
            2: "east_wall",
        ...
        for link_idx, wall_name in wall_links.items():
            try:
                p.changeVisualShape(self.ROOM_ID,
                                ...
                                physicsClientId=self.CLIENT)
                success_count += 1
                print(f"[DEBUG] ✓ Applied to {wall_name} (link {link_idx})")
                
            except Exception as e:
                print(f"[DEBUG] ✗ Failed to apply to {wall_name} (link {link_idx}): {e}")
        
        return success_count

    def _applyWallTexturesByScanning(self, wall_texture_id):
        try:
            num_joints = p.getNumJoints(self.ROOM_ID, physicsClientId=self.CLIENT)
            success_count = 0
            
            for i in range(num_joints):
                try:
                    # Get link info
                    joint_info = p.getJointInfo(self.ROOM_ID, i, physicsClientId=self.CLIENT)
                    ...
                    if "wall" in link_name.lower():
                        p.changeVisualShape(self.ROOM_ID,
                                        ...
                                        physicsClientId=self.CLIENT)
                        success_count += 1
                        print(f"[DEBUG] ✓ Scanned and applied to {link_name} (link {i})")
            ...
            print(f"[DEBUG] Link scanning failed: {e}")
            return 0

    def _printMazeLinkInfo(self):
        if not hasattr(self, 'ROOM_ID'):
            return
            
        try:
            num_joints = p.getNumJoints(self.ROOM_ID, physicsClientId=self.CLIENT)
            print(f"\n[DEBUG] === MAZE URDF STRUCTURE ===")
            print(f"[DEBUG] Total links: {num_joints + 1} (including base)")
            print(f"[DEBUG] Base link (-1): floor")
            
            for i in range(num_joints):
                try:
                    joint_info = p.getJointInfo(self.ROOM_ID, i, physicsClientId=self.CLIENT)
                    link_name = joint_info[12].decode() if joint_info[12] else f"unnamed_link_{i}"
                    joint_name = joint_info[1].decode() if joint_info[1] else f"unnamed_joint_{i}"
                    print(f"[DEBUG] Link {i}: {link_name} (joint: {joint_name})")
                except:
                    print(f"[DEBUG] Link {i}: <error getting info>")
            
            print(f"[DEBUG] === END STRUCTURE ===\n")
                    
        except Exception as e:
            print(f"[DEBUG] Error getting maze link info: {e}")

    def _validateTextures(self):

        texture_paths = {
            'floor': r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\floortexture.png",
            'wall': r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Walltexture.jpeg"
        }
        
        results = {}
        
        for texture_type, path in texture_paths.items():
            results[texture_type] = {
                'path': path,
                'exists': os.path.exists(path),
                'filename': os.path.basename(path) if os.path.exists(path) else None,
                'size': os.path.getsize(path) if os.path.exists(path) else 0
            }
            
            if results[texture_type]['exists']:
                print(f"[INFO] ✓ {texture_type.title()} texture found: {results[texture_type]['filename']} ({results[texture_type]['size']} bytes)")
            else:
                print(f"[WARNING] ✗ {texture_type.title()} texture missing: {path}")
    ...
    ################################################################################
    ####################Obstacle with Interior######################################
    ################################################################################

    # Modified BaseAviary methods - Add these to your existing BaseAviary class

    """def _addObstacles(self):
        # Load the enhanced indoor environment URDF
        indoor_urdf_path = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\RoomNoInterior Triangle.urdf"
        
        try:
            self.INDOOR_ENV_ID = p.loadURDF(indoor_urdf_path, physicsClientId=self.CLIENT)
            print(f"[INFO] ✓ Successfully loaded indoor environment with ID: {self.INDOOR_ENV_ID}")
            
            # Apply textures to the environment
            self._setupIndoorTextures()
            
            # Set up dynamic obstacles (optional moving objects)
            self._setupDynamicObstacles()
            
            # Print environment layout info
            if self.GUI:
                self._printIndoorEnvironmentInfo()
                
        except Exception as e:
            print(f"[ERROR] Failed to load indoor environment from {indoor_urdf_path}: {e}")
            print("[INFO] Falling back to basic environment")
            self._loadBasicEnvironment()

    def _setupIndoorTextures(self):
        if not hasattr(self, 'INDOOR_ENV_ID'):
            print("[WARNING] No indoor environment loaded, cannot apply textures")
            return
        
        print("[INFO] Setting up indoor environment textures...")
        
        # Define texture paths
        texture_paths = {
            'floor': r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\concrete-tile-background.jpg",
            'wall': r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Walltexture.jpeg"
        }
        
        # Apply textures
        floor_success = self._applyFloorTexture(texture_paths['floor'])
        wall_success = self._applyWallTextures(texture_paths['wall'])
        furniture_success = self._applyFurnitureTextures()
        
        # Summary
        texture_results = [floor_success, wall_success, furniture_success]
        successful_textures = sum(texture_results)
        
        if successful_textures == 3:
            print("[INFO] ✓ All textures applied successfully!")
        else:
            print(f"[INFO] Applied {successful_textures}/3 texture categories successfully")
    ...
    def _applyFloorTexture(self, texture_path):
        try:
            ...
            # Apply to base link (floor) - linkIndex=-1 for base link
            p.changeVisualShape(self.INDOOR_ENV_ID, 
                            linkIndex=-1,  # base_link is the floor
                            textureUniqueId=texture_id,
                            rgbaColor=[1, 1, 1, 1],  # White tint for clear texture
                            physicsClientId=self.CLIENT)
    ...
    def _applyWallTextures(self, wall_texture_path):
        ...
        try:
            ...
            # Apply to wall links (links 0-3 are typically the outer walls)
            wall_links = [0, 1, 2, 3]  # north, south, east, west walls
            success_count = 0
            
            for link_idx in wall_links:
                try:
                    p.changeVisualShape(self.INDOOR_ENV_ID,
                                    linkIndex=link_idx,
                                    textureUniqueId=wall_texture_id,
                    ...
                    print(f"[DEBUG] Could not apply texture to wall link {link_idx}: {e}")
    ...
    def _applyWallTexturesByScanning(self, wall_texture_id):
        try:
            num_joints = p.getNumJoints(self.INDOOR_ENV_ID, physicsClientId=self.CLIENT)
            success_count = 0
            
            for i in range(num_joints):
                try:
                    joint_info = p.getJointInfo(self.INDOOR_ENV_ID, i, physicsClientId=self.CLIENT)
                    link_name = joint_info[12].decode() if joint_info[12] else f"link_{i}"
                    ...
                    if "wall" in link_name.lower():
                        p.changeVisualShape(self.INDOOR_ENV_ID,
                                        ...
                                        physicsClientId=self.CLIENT)
                        success_count += 1
                        print(f"[DEBUG] ✓ Applied wall texture to {link_name} (link {i})")
            ...
            print(f"[DEBUG] Wall texture scanning failed: {e}")
            return 0

    def _applyFurnitureTextures(self):
        if not hasattr(self, 'INDOOR_ENV_ID'):
            return False
        
        try:
            # Define furniture colors based on typical link indices
            furniture_colors = {
                # Table and wooden furniture (brown wood color)
                4: [0.6, 0.4, 0.2, 1],   # conference_table_top
                9: [0.6, 0.4, 0.2, 1],   # desk
                13: [0.5, 0.3, 0.1, 1],  # bookshelf
                
                # Metal furniture (gray metal)
                5: [0.7, 0.7, 0.7, 1],   # table_leg_1
                6: [0.7, 0.7, 0.7, 1],   # table_leg_2
                7: [0.7, 0.7, 0.7, 1],   # table_leg_3
                8: [0.7, 0.7, 0.7, 1],   # table_leg_4
                16: [0.6, 0.6, 0.6, 1],  # printer_stand
                
                # Fabric furniture (blue fabric)
                11: [0.2, 0.2, 0.8, 1],  # chair_1
                12: [0.2, 0.2, 0.8, 1],  # chair_2
                17: [0.4, 0.2, 0.1, 1],  # sofa (brown leather)
                
                # Other furniture
                10: [0.8, 0.8, 0.9, 1],  # tall_cabinet (light gray)
                15: [0.9, 0.9, 1.0, 0.7], # coffee_table (glass)
                18: [0.1, 0.6, 0.1, 1],  # floor_plant (green)
            }
            
            success_count = 0
            for link_idx, color in furniture_colors.items():
                try:
                    p.changeVisualShape(self.INDOOR_ENV_ID,
                                    linkIndex=link_idx,
                                    rgbaColor=color,
            ...
            if success_count > 0:
                print(f"[INFO] ✓ Applied colors to {success_count} furniture pieces")
                return True
            else:
                print("[INFO] Using default furniture materials")
                return False
                
        except Exception as e:
            print(f"[DEBUG] Furniture coloring failed: {e}")
            return False

    def _setupDynamicObstacles(self):
        try:
            # Add some movable obstacles (boxes, equipment)
            self.dynamic_obstacles = []
            
            # Movable boxes (packages, equipment)
            box_positions = [
                [2.0, -1.0, 0.25],   # Near conference table
                [-4.0, 1.0, 0.25],   # Near bookshelf
                [4.0, -4.0, 0.25]    # Corner area
            ]
            
            for i, pos in enumerate(box_positions):
                try:
                    # Create a simple box obstacle
                    box_collision = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.15, 0.15, 0.25])
                    box_visual = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.15, 0.15, 0.25],
                                                rgbaColor=[0.8, 0.6, 0.4, 1])  # Cardboard brown
                    
                    box_id = p.createMultiBody(baseMass=1.0,
                                            baseCollisionShapeIndex=box_collision,
                                            baseVisualShapeIndex=box_visual,
                                            basePosition=pos,
                                            physicsClientId=self.CLIENT)
                    
                    self.dynamic_obstacles.append(box_id)
                    
                except Exception as e:
                    print(f"[DEBUG] Could not create dynamic obstacle {i}: {e}")
            
            if len(self.dynamic_obstacles) > 0:
                print(f"[INFO] Added {len(self.dynamic_obstacles)} dynamic obstacles")
            
        except Exception as e:
            print(f"[DEBUG] Could not add dynamic obstacles: {e}")
            self.dynamic_obstacles = []

    def _printIndoorEnvironmentInfo(self):
        if not hasattr(self, 'INDOOR_ENV_ID'):
            return
        
        print(f"\n[DEBUG] === INDOOR ENVIRONMENT LAYOUT ===")
        print(f"[DEBUG] Environment ID: {self.INDOOR_ENV_ID}")
        print(f"[DEBUG] Room Size: 15m x 15m x 3m")
        print(f"[DEBUG] Dynamic Obstacles: {len(getattr(self, 'dynamic_obstacles', []))}")
        
        # Print link structure for debugging
        try:
            num_joints = p.getNumJoints(self.INDOOR_ENV_ID, physicsClientId=self.CLIENT)
            print(f"[DEBUG] Total furniture pieces: {num_joints}")
            print(f"[DEBUG] Base link (-1): Floor")
            ...
            for i in range(min(10, num_joints)):
                try:
                    joint_info = p.getJointInfo(self.INDOOR_ENV_ID, i, physicsClientId=self.CLIENT)
                    link_name = joint_info[12].decode() if joint_info[12] else f"link_{i}"
                    print(f"[DEBUG] Link {i}: {link_name}")
                except:
                    print(f"[DEBUG] Link {i}: <info unavailable>")
            
            if num_joints > 10:
                print(f"[DEBUG] ... and {num_joints - 10} more furniture pieces")
                
        except Exception as e:
            print(f"[DEBUG] Error getting environment info: {e}")
        
        print(f"[DEBUG] === END ENVIRONMENT INFO ===\n")

    def _loadBasicEnvironment(self):
        try:
            # Load the basic room as fallback
            basic_urdf_path = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Room.urdf"
            self.INDOOR_ENV_ID = p.loadURDF(basic_urdf_path, physicsClientId=self.CLIENT)
            print("[INFO] Loaded basic room environment as fallback")
            
            # Apply basic textures
            floor_texture = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\floortexture.png"
            wall_texture = r"D:\Documents\ITB\S2\Semester 2\Thesis 2\gym-pybullet-drones\gym_pybullet_drones\envs\Walltexture.jpeg"
            
            self._applyFloorTexture(floor_texture)
            self._applyWallTextures(wall_texture)
            
        except Exception as e:
            print(f"[ERROR] Failed to load even basic environment: {e}")

    def getIndoorEnvironmentInfo(self):
        if not hasattr(self, 'INDOOR_ENV_ID'):
            return None
        
        try:
            num_joints = p.getNumJoints(self.INDOOR_ENV_ID, physicsClientId=self.CLIENT)
            base_pos, _ = p.getBasePositionAndOrientation(self.INDOOR_ENV_ID, physicsClientId=self.CLIENT)
            
            environment_info = {
                'environment_id': self.INDOOR_ENV_ID,
                'base_position': base_pos,
                'total_furniture_pieces': num_joints,
                'dynamic_obstacles': len(getattr(self, 'dynamic_obstacles', [])),
                'room_bounds': {
                    'x_min': -7.5, 'x_max': 7.5,
                    'y_min': -7.5, 'y_max': 7.5,
                    'z_min': 0.1, 'z_max': 2.8
                },
                'recommended_flight_height': 1.5,
                'max_safe_speed': 2.0
            }
    ...
    def checkCollisionAtPosition(self, position, drone_radius=0.1):
        if not hasattr(self, 'INDOOR_ENV_ID'):
            return True  # No environment loaded, assume safe
        
        try:
            # Create a temporary sphere at the position to test for collision
            temp_sphere_id = p.createCollisionShape(p.GEOM_SPHERE, radius=drone_radius)
            temp_body_id = p.createMultiBody(baseMass=0.01,
                                        baseCollisionShapeIndex=temp_sphere_id,
                                        basePosition=position,
                                        physicsClientId=self.CLIENT)
            
            # Step simulation once to detect collisions
            p.stepSimulation(physicsClientId=self.CLIENT)
            
            # Check for contacts with the environment
            contacts = p.getContactPoints(bodyA=temp_body_id, 
                                        bodyB=self.INDOOR_ENV_ID,
                                        physicsClientId=self.CLIENT)
            
            # Also check contacts with dynamic obstacles
            for obstacle_id in getattr(self, 'dynamic_obstacles', []):
                obstacle_contacts = p.getContactPoints(bodyA=temp_body_id,
                                                    bodyB=obstacle_id,
                                                    physicsClientId=self.CLIENT)
                contacts.extend(obstacle_contacts)
            
            # Remove the temporary body
            p.removeBody(temp_body_id, physicsClientId=self.CLIENT)
            
            # Return True if no collisions detected
            return len(contacts) == 0
            
        except Exception as e:
            print(f"[DEBUG] Collision check failed: {e}")
