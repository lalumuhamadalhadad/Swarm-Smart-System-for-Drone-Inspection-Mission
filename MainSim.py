...
# ============================================================================
ENABLE_FOLLOW_CAMERA = False  # Set to True to enable follow camera
...
def get_waypoint_configuration(external_waypoints=None):
    ...
    waypoint_mode = "TWO_DRONE"  # Change this to switch waypoint sets
...
# Drones: 2 | Grid: 18x5 | Formation: corner
...
class ControlEvaluationPlotter:
    ...
    def plot_pid_speed_performance(self, simulation_data, save_path=None):
        ...
        for j in range(self.num_drones):
            if j in simulation_data.pid_data and simulation_data.pid_data[j]['target_speed']:
                ts = np.asarray(simulation_data.pid_data[j]['target_speed'], dtype=float)
                act = np.asarray(simulation_data.pid_data[j]['speed_command'], dtype=float)

                # align lengths just in case
                n = min(len(ts), len(act), len(simulation_data.data['simulation_time']))
                ts  = ts[:n]
                act = act[:n]
                time_data = np.asarray(simulation_data.data['simulation_time'][:n], dtype=float)

                target_speeds = ts * 0.67  # scaled dynamic target

                ax.plot(time_data, target_speeds, '--', color="#000000",
                        label=f'Drone {j} Dynamic Target', alpha=0.7, linewidth=4)
                ax.plot(time_data, act, '-', color=self.drone_colors[j],
                        label=f'Drone {j} Actual', linewidth=2)

        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Speed (m/s)')
        ax.legend(loc='upper right', framealpha=0.6, facecolor='white', fontsize=14)
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        if save_path:
            plt.savefig(f"{save_path}/dynamic_pid_performance_evaluation.png", dpi=600, bbox_inches='tight')
        plt.show()
    ...
    def plot_obstacle_avoidance_performance(self, simulation_data, min_safe_distance=0.5, save_path=None):
        ...
        if save_path:
            plt.savefig(f"{save_path}/obstacle_avoidance_evaluation.png", dpi=600, bbox_inches='tight')
...
def run(
    ...
    if gui:  # Remove the ENABLE_FOLLOW_CAMERA check here
        follow_camera = DroneFollowCamera(
            target_drone_id=1,  # Follow drone 0 by default
            distance=5.0,
            height_offset=2.0,
            smooth_factor=0.1
        )
